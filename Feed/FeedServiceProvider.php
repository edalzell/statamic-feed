    ...
    public function boot()
    {
        ...
        foreach ($old_routes as $i => $route) {
            if ($route->getUri() == '{segments?}') {
                $json_feed_route = new Route(['GET'], 'feed.json', ['uses' => '\Statamic\Addons\Feed\FeedController@json']);
                $new_routes->add($json_feed_route);
                $atom_feed_route = new Route(['GET'], 'feed', ['uses' => '\Statamic\Addons\Feed\FeedController@atom']);
